...
istream& operator >> (istream& input, kitob& k)
{
    cout << "Kitob nomini kiriting\n";
    input >> k.nomi;

    cout << "Muallif familiyani kiriting\n";
    input >> k.familiya;

    cout << "Muallif ismni kiriting\n";
    input >> k.ism;

    cout << "Chop etilgan yilini kiriting\n";
    input >> k.yili;

    return input;
}
// chiqarish amalini qayta yuklash
ostream& operator << (ostream& out, kitob k)
{
    out << k.nomi;
    out << "\t" << k.familiya;
    out << "\t" << k.ism;
...
int main()
{
    // kitob sinfidagi ob'ektni e'lon qilish
    kitob p;
    FILE * f;
    int n = 0;

    f = fopen("kitoblar.dat", "a+");

    cout << "Faqat ro'yxatni ko'rish uchun 0 kiriting\n";
    cout << "Yangi kiritiladigan kitoblar sonini kiriting\n"; cin >> n;

    if (n >= 1)
    for (int i = 1; i <= n; i++)
    {
        cout << i << " - kitob ma'lumotlarini kiriting\n";
        cin >> p;

        // faylga ma'lumotlarni yozish
        fwrite(&p, sizeof(kitob), 1, f);
    }
    // fayl boshiga o'tish
    rewind(f);

    cout << "Kitoblar ro'yxati\n";
    n = 0;
    while (fread(&p, sizeof(kitob), 1, f))
    {
        n++;
        cout << n << " ";
        cout << p;
    ...
    n = 0;
    cout << "2012 yil chop etilgan kitoblar\n";
    while (fread(&p, sizeof(kitob), 1, f))
    {
        if ( p.yili == 2012)
        {
