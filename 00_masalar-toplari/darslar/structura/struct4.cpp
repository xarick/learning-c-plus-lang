...
istream& operator >> (istream& input, talaba& k)
{
...
ostream& operator << (ostream& out, talaba k)
{
    out << k.familiya;
    out << " " << k.ism << "\t";
    for (int i = 0; i < 4; i++)
...
int main()
{
    // talaba sinfidagi ob'ektni e'lon qilish
    talaba p;
    FILE * f;
    int n = 0;

    f = fopen("Talabalar.dat", "a+");

    cout << "Faqat ro'yxatni ko'rish uchun 0 kiriting\n";
    cout << "Yangi kiritiladigan talabalar sonini kiriting\n"; cin >> n;

    if (n >= 1)
    for (int i = 1; i <= n; i++)
    {
        cout << i << " - talaba ma'lumotlarini kiriting\n";
        cin >> p;

        // faylga ma'lumotlarni yozish
        fwrite(&p, sizeof(talaba), 1, f);
    }
    // fayl boshiga o'tish
    rewind(f);
    ...
    n = 0;
    while (fread(&p, sizeof(talaba), 1, f))
    {
        n++;
        cout << n << " ";
        cout << p;
    ...
    // fayl boshiga o'tish
    rewind(f);
    cout << "Faqat a'lo baxolarga o'qigan talabalar ro'yxati\n";
    n = 0;
    while (fread(&p, sizeof(talaba), 1, f))
    {
        if ((p.b[0] + p.b[1] + p.b[2] + p.b[3]) == 20)
        {
            n++;
            cout << n << " ";
            cout << p;
...
// -> bularni diqqat bilan o'qib chiqin 
// structura
// istream& operator >> (istream& input, talaba& k) obbyektga yonaltirilgan dasturlar uchun qayta kirgizish
// 
// manli fayllar
// !!! ofstream yoz("2.txt",ios::app) matnni ochirmasdan davomidan yozish
// eng.assign(s,0,p-1); eng ozgaruvchisiga 0 inci elementdan boshlab p-1 ta element nusxalandi
// fromfile.read(buffer, length); bolkka malumotlani oqish
// delete[] buffer; hotirani boshatish
// buffer = new char [length]; hotira ajratish
// fromfile.seekg(0, ios::beg); // fayl boshiga otish
// fromfile.seekg(0, ios::end); // fayl ohririga otish
// length=fromfile.tellg(); fayl hajmi aniqlanadi
// ofstream tofile(yangi, ios::binary); yozish uchun binar fayl ochish
// long tellp() yozsih oqimnig kelgan jooyini aniqlash 
// ios::beg oqimning boshlannishi
// ios::cur oqimning joriy holati
// ios::end oqim oxiri 
// oqish.eof() satr ohiri
// while(!oqish.eof()) satr ohiriga yetmagan paytda
// if(!oqish.is_open()) agar fayl ochilmasa deganni bildiradi
// yozish.open("matn01.txt"); shu matnga yozish
// yozish.close(); yozish uchun ochilgan faylni yopish
// oqish.close(); oqish uchun ochilgan faylni yopish
// ofstream ios::out
// ifstream ios::in
// fstream ios::in|ios::out
// har bir sinf uchun mode parametrinig odatiy qiymatlari mavjud
// ios::in -> faqat malumot o'qish uchun
// ios::out -> faqat malumot yozish uchun ishlatiladi
// ios::ate -> faylni ochishda fayl korsatkichini fayl  ohiriga qoyish uchun ishlatiladi
// ios::app -> fayl ohiriga malumot yozish uchun ishlatiladi
// ios::trunc -> bor bo'lgan faylni ustidan yangi fayl yozish
// ios::binary -> binar holda malumotlarni almshtirish uchun ishlatiladi
// #include <fstream> kutubhona qoshiladi
// fstream fayldan malumot o'qish va yozish uchun ishlatiladi fstream yoz_oqi;
// ifstream fayldan malumot o'qish uchun ishlatiladi ifstream oqi;
// ofstream faylga malumot yozish uchun ishlatiladi ofstream yoz;
// binar fayllar
// fseek(f,-sizeof(int),SEEK_CUR); cursorni siljitish
// fwrite(&k,sizeof(int),1,f); yozish
// fseek(f, -sizeof(int), SEEK_CUR); chapga surish
// fseek(f,ftell(f),SEEK_SET);  siljitish
// fseek(f,ftell(f),SEEK_SET); fayl korsatkichini ornatib qoyish
// fseek(f, 0, SEEK_END);  fayl ohiriga otish
// SEEK_SET  fayl boshiga nisbatan siljish
// SEEK_CUR  fayl joriyga nisbatan siljish
// SEEK_END  fayl ihiriga nisbatan siljish
// fseek fayl ni siljitish uchun ishlatiladi
// ftell(f); bu funksiya orqali fayl hajmi qancha ekanlii koriladi
// rewind(f) -> fayl korsatkichini fayl bshiga ornatadi
// feof(f) fayl ohiri
// fread(&k,sizeof(k),1,f); faylni oqish
// fwrite(&k,sizeof(k),1,f);  yozish
// fclose(f); faylniyopish uchun ishlatiladi
// binar fayllar uchun #include <stdio.h>  kutubhona ishlatiladi
// FILE *f; elon qilish
// f=fopen("1.dat","wb"); yozish uchun fayl ochish
// f=fopen("1.dat","wb+"); yozish va oqish uchun fayl ochish
// rekursiya
// oz ozini chaqiruvchi funksiyalar
// if(k==0||k==1)
// return 1;
// return k*fact(k-1);
// string!!!
