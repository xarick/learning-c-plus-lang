...
{
char s[50], amal[30];
int sum=0, son[10], sonlar_soni=0,amallar_soni=0;
...
{
    if(s[i]=='+' || s[i]=='-')
    amal[amallar_soni++]=s[i];    
}
// satr ohirini qoshish
amal[amallar_soni]='\0';
char *p;
p=strtok(s,"+-");
// sonlar alohida massivga ajraatib olamiz
cout<<"satrda ishlatilgan sonlar"<<endl;
while(p!=NULL)
{
     son[sonlar_soni]=atoi(p);
     cout<<son[sonlar_soni++]<<endl;
     p=strtok(NULL,"+-");            
}
sum=son[0];
cout<<"bajarilgan  amallar"<<endl;
for(int i=0; i<amallar_soni; i++)
{
    cout<<amal[i]<<endl;
    switch(amal[i])
    {
