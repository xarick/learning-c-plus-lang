...
// 1-probelni topish
bir=s.find(" ");
// 1-probeldan keyingi belgidan boshlab qidiramiz
bir++;
// 2-probelni topish
ikki=s.find(" ",bir);
// probel  yoq bolsa yoki faqat 1 ta probel bolsa
// bosh satr chiqadi
if (s.find(" ")>s.length() || s.rfind(" ")==s.length())
    cout<<endl; // bosh satr chiqarish
else
    cout<<s.substr(bir,ikki-bir)<<endl;
getch(); //getchar();
// system("PAUSE");
return 0;
}
