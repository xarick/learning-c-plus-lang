...
string s,c[20],x;
int k=0;
cout<<"satrni kiritinng\n";
getline(cin,s);
//satr ohirigaa probel qoshamiz
s+=" ";
// har bir pribel belgilarni soz deb qabul qilamiz
// c-massivga ozlashtiramiz
for(int i=0 ;i<s.length(); i++)
{ 
   // probel uchrasa yangi soz
   if(s[i]==' ')
   {
      c[k++]=x;
      // x yordamchi satrini tozalaymiz
      x.clear(); // x="";        
   }
        // probel uchramasa x satriga belgilar yigib boramiz
        else
        x+=s[i];
}
cout<<"\nbir hil harf bialn boshlanuvchi va tugovchi sozlar"<<endl;
int soni=0;
for(int i=0; i<k; i++)
{
  int oxiri=c[i].length()-1;
  // birinchi va ohirgi belgilarning tenngligi tekshirilsin
  if(c[i][0]==c[i][oxiri])
  {
      cout<<c[i]<<endl;
      soni++;                    
...
cout<<soni<<" ta talab qilingan soz bor"<<endl;
getch(); //getchar();
// system("PAUSE");
return 0;
}
